...
- **Theme:** Data-Dense Dashboard, **light mặc định** (đổi từ dark 2026-08-18 theo yêu cầu user;
  ...
  destructive `#DC2626`, nền light `#F1F5F9` / dark `#0F172A`. Font Fira Sans (UI) + Fira Code (số).
  Icon Lucide qua package `lucide_icons_flutter` (KHÔNG dùng `lucide_icons` — package đó extend
  `IconData` final nên dart2js từ chối, `flutter build web` fail dù analyze/test pass).
...
- [x] Realtime toàn app (2026-08-20): migration 20260820000000 + `realtimeRefreshProvider`; auth autoconfirm + site_url; UI: tint giao dịch xanh dương/xám 2 tone so le, dashboard tap-to-edit, form báo lỗi lưu/xóa
- [x] Release workflow (2026-08-20): `.github/workflows/release.yml` — push tag `v*` → build APK (ubuntu) + Windows zip (windows runner) → đính vào GitHub Release; publishable key hardcode trong workflow (public by design); APK debug-signed (đủ cho sideload)
...
- [ ] Tải APK từ GitHub Releases, cài lên điện thoại
...
- Multi-currency accounting đầy đủ (hiện: tổng/số dư/net worth chỉ tính account USD, có ghi chú khi bị loại; quy đổi hiển thị VND có sẵn), shared/family accounts, iOS, push notification (cần Firebase project — user tự tạo), import OFX, AI hỏi đáp (tốn phí API — phá ràng buộc $0, cần user quyết)
- Verify JWS webhook vẫn deferred (đã thêm cooldown 45s/item để chặn spam); updateTxnMeta là last-writer-wins toàn phần (accepted risk — app 1 người dùng)
...
|---|---|---|---|
| 2026-08-20 | Claude Code | Release + docs: workflow `release.yml` (tag `v*` → APK + Windows zip → GitHub Release, `flutter create` platform folders ngay trong runner); README cập nhật link web live, mục Releases, thứ tự 6 migration, ghi chú wrangler `--branch=main`; tag `v0.1.0` đầu tiên | Kiểm tra release build xanh, tải APK cài thử |
| 2026-08-20 | Claude Code | Hậu deploy: (1) auth config — bật `mailer_autoconfirm`, `site_url`→pages.dev (SMTP free ~2 mail/giờ dễ tưởng app hỏng); (2) tài khoản test `test@example.com` + item sandbox First Platypus Bank tạo qua `/sandbox/public_token/create` (bank OAuth tên thật lỗi trong sandbox — hướng dẫn user dùng Platypus/user_good); sync 14 account + 48 txn OK; (3) UI: tô nền giao dịch vào/ra (xanh chuối non `#EAF5D9` / xám `#F3F4F6`, có biến thể dark), dashboard click txn mở form sửa, form lưu/xóa có try/catch báo lỗi; (4) **realtime toàn app** — migration 20260820000000 thêm 6 bảng vào publication (KHÔNG thêm plaid_items: payload realtime lộ access_token qua mặt column grant), `realtimeRefreshProvider` 1 channel + debounce 800ms invalidate provider theo bảng, giữ sống trong AppShell; E2E verify qua WS thật. Lưu ý vận hành: deploy Pages phải `--branch=main` (wrangler lấy branch git → rơi vào preview URL); lỗi JWT phía user do session cũ hỏng vì lệch giờ máy → đăng xuất/đăng nhập lại | Theo dõi user test thực tế; chuyển PLAID_ENV=production khi được Plaid duyệt |
...
| 2026-08-19 | Claude Code | Sửa 27/29 finding từ /code-review 7-agent + refactor. Sync: item lỗi được retry (status `login_required` mới cho ITEM_LOGIN_REQUIRED + banner "Đăng nhập lại" dùng Link update-mode qua `link_tokens.item_id`); pending→posted migrate giữ category/note/split (`pending_transaction_id`); account mới tự tạo khi sync (fail-loudly nếu vẫn lạ, trước khi lưu cursor); cursor lưu bằng CAS, MUTATION_DURING_PAGINATION coi là retryable; syncNow đọc lỗi từng item; webhook cooldown 45s. Data: mọi provider watch `currentUserIdProvider`; phân trang `_fetchAll` vượt trần 1000 dòng; models chịu payload realtime; replica identity full; RPC `split_transaction` (atomic + validate tổng + FOR UPDATE, đã smoke-test PG16); repairSplits xóa children lệch tổng khi sync sửa amount; USD-only cho tổng/snapshot/forecast; toUtc. Parsing: `core/parsing.dart` chung (amount VN/EU, dd/MM detect + dropdown, form validate, clamp date picker, clamp cuối tháng recurring); rollover chained 12 tháng; escape LIKE trong applyRuleToExisting. Refactor: categoryNamesProvider, recurringItems/AllProvider derived, cache NumberFormat, export Future.wait+paginate. 2 mục không sửa (ghi ở Deferred). Verify: analyze sạch, 32 test pass, deno check pass, 5 migration + RPC pass PG16 | Chờ CI xanh rồi merge main |
